...
- **⚠️ In-card controls — navigation = LINK, action = BUTTON (Balzhima, 2026-07, Jakob's law).** The old mono/UPPERCASE tonal pill (`CardButton`) is **retired**: mono+uppercase+tracking is this DS's *label* language (CURRENT NET WORTH, ALLOCATION), so an action wearing it read as a label; the tonal pill read as a *badge*; `text-muted` made it the quietest thing in the card. Its Figma master ("Button-inside-card" `1428:14011`) is gone, so it wasn't DS-backed. Now: **`CardLink`** ("See all"/"See more" = navigation) is a plain **text link** — 14px Medium, sentence case, `text-secondary` → `text-ink` on hover, + chevron, no container. **In-card actions** ("Set targets", "Edit targets", "Edit") use the **real `<Button size="s" variant="tonal">`** — a button inside a card must look like every other button. `CardButton` was deleted from `CardControl.tsx` + the barrel. **`CardDropdown` keeps the mono pill on purpose** — it surfaces a *value* (`VALUE ↓`, the active sort field), which IS the tabular/label language.
