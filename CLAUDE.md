...
- **⚠️ Both in-card Figma masters EXIST and were re-synced to the code (2026-07)** — an earlier note here wrongly claimed `1428:14011` was deleted (a `get_design_context` "invalid node selection" error just means the node isn't on the current page). **`SortDropdown` `1428:13993`** (Forms page) → now `Sort: VALUE ↓`, 109×28; its Chevron instance was swapped for a **Geist Mono `↓`** because a chevron re-introduces the "opens a menu" reading. **`Button-inside-card` `1428:14011`** → restyled to mirror `Button (Size=Small, Type=Tonal)` (27px, 14px SemiBold, `btn-tonal`, `text-primary`, trailing hidden). It was **not deleted** — it has **15 live instances** across screen frames, which all inherit the restyle. New Figma work should instance the real **Button** set directly.
