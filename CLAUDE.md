...
### Button size scale — H-pad is a DELIBERATE EXCEPTION to the snap (2026-07)
The code `Button` (`src/components/william/Button.tsx`) takes `size?: 'l'|'m'|'s'|'xs'` (default `m`); `xs` is a **code-only** inline size with no Figma peer. **H-pad walks the Spacing scale 12 → 16 → 24** (`spacing/3` · `spacing/4` · `spacing/6`), hand-set in both Figma and code:
| Size | Height | H-pad | Label | Icon | Used for |
|---|---|---|---|---|---|
| **L** | **42px** | px-6 (24) | 14px SemiBold | 20 | prominent CTAs — modal confirm/cancel, empty-state + dashboard "Add trade" |
| **M** | **36px** | px-4 (16) | 14px SemiBold | 18 | **default** — Portfolio toolbar pills (Refresh/Import/Add trade) |
| **S** | **27px** | px-3 (12) | 14px SemiBold | 16 | compact secondary — Set/Edit targets |
...
- **⚠️ Why this is hand-set, not snapped — do NOT "correct" it.** The mechanical snap sent L's 20 and S's 18 both to 16, where M already sat, so all three sizes ended up with identical padding and were separated by height alone. Worse, Figma's *Neutral* variants had landed on 12 while *Primary* landed on 16, so the two sides disagreed with each other. The 12/16/24 ladder is the fix: every step is a real Spacing token, and padding scales with size again. Applied to the `Button` set (`897:7790`) **and** the `Danger button` set (`899:7421`) — 105 variants, bound to spacing vars, Primary and Neutral now identical.
- ⚠️ **Remaining Figma↔code divergence:** the masters' **Small renders 25px tall**, code says **27px**. Figma's height HUGs its (snapped) vertical padding; code sets `h-[27px]` explicitly. 2px, unreconciled — pick one when convenient. Large is also 42 (Button) vs 44 (Danger) on both sides, a pre-existing inconsistency.
...
- **A token scale nobody used → enforced spacing (2026-07).** Strongest "systems" thread so far, and the most honest. **Problem:** the Spacing collection existed but only **19 properties in the entire Figma file** were bound to it; ~61% of screen spacing was off-scale, and the single most-used value (20, ×401 — the standard card padding) *wasn't even a token* — the ramp skips 16→24. The scale described nothing. **Principle:** a design system's constraint is only real if it's enforced; an unenforced token is documentation, not a system. **Before → after:** 8,360 properties bound (from 19), 1,047 Figma nodes + 252 Tailwind classes moved, card padding 20→16 across every master and screen. **The interesting part — where the mechanical rule broke down and judgment had to take over:** (1) 20/14/10/6 were all *exact midpoints* between tokens, so "snap to nearest" was a coin flip ~990 times — a rule that can't decide isn't a rule, someone has to pick (we picked "round down"). (2) The snap **destroyed the Button size scale** — L(20) and S(18) both collapsed onto M's 16, and Figma's Neutral/Primary variants disagreed with each other — so it was hand-set back to 12/16/24, still all real tokens. *The exception is the point: systems serve the design, not the reverse.* (3) It **surfaced latent bugs the old spacing was hiding** — `Card / Milestones` had been clipping its year labels forever (40px frame, 49px content); the 2px gaps just pushed them past the clip edge. **Tradeoffs accepted:** three of seven per-breakpoint dashboard gaps collapsed (18 and 20 both → 16), so mobile≠desktop stopped being true there. Also a good "working with devs" beat: the same rule was applied to Figma *and* the codebase in one pass, and the two sides were diffed against each other to prove they agreed.
